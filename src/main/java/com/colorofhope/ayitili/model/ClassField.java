...
public class ClassField {
    ...
    private static String HTML_ENUM =
            "<div class=\"form-group\">\n" +
                    ...
                    "</select>\n" +
                    "</div>";
    private static String HTML_ENUM_OPTION =
            "<option th:value=\"%1$s\">%2$s</option>\n";
    ...
    public ClassField(String name, Class type, String bootstrapLabel) {
        ...
        if (classToHtmlMap.get(name) != null) {
            ...
            Stream<String> options = Arrays.stream(type.getEnumConstants()).map(enumValue -> {
                BootstrapLabel bootstrapOptionLabel = null;
                try {
                    bootstrapOptionLabel = type.getField(enumValue.toString()).getAnnotation(BootstrapLabel.class);
                } catch (NoSuchFieldException e) {
                    e.printStackTrace();
                }
                return String.format(HTML_ENUM_OPTION, enumValue, bootstrapOptionLabel == null ? enumValue : bootstrapOptionLabel.value());
            });
    ...
    public ClassField(Field field, Object object) {
        ...
        String htmlTemplate = bootstrapHtmlDisplay == null ? HTML_IGNORE_DIV : bootstrapHtmlDisplay.value();
